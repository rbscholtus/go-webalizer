...
type Stats struct {
	...
	sites          int            // Requests originating from 'site', referenced by a name or IP address
...
func (s Stats) String() string {
	output := fmt.Sprintf("Stats:\n"+
		...
		"  Visits: %d\n"+
		"  Bytes:  %d bytes\n",
		s.hits, s.files, s.pages, s.sites, s.visits, s.bytes)
...
func processLog(fileName string) {
	...
	for scanner.Scan() {
		...
		// any GET request sends something back
		if match[5] == "GET" {
			stats.files++
